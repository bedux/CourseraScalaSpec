...
class BlurSuite extends FunSuite {
  ...
  test("Blur boxBlurKernel") {
    ...
    assert(boxBlurKernel(src, 0, 0, 1) === (3+4+1)/4,
          s"(boxBlurKernel(0, 0, 1) should be 2, " +
            s"but it's ${boxBlurKernel(src, 0, 0, 1)})")

    assert(boxBlurKernel(src, 2, 2, 1) === (5+4+7+8)/4,
      s"(boxBlurKernel(0, 0, 1) should be 2, " +
        s"but it's ${boxBlurKernel(src, 0, 0, 1)})")
  ...
  test("VerticalBoxBlur.blur with radius 3 should correctly blur the entire Parallel " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
